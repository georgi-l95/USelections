...
describe("USElection", function () {
  ...
  it("Should throw when try to submit with zero seats", async function () {
    const stateResult = ["Montana", 1000, 900, 0];
    expect(usElection.submitStateResult(stateResult)).to.be.revertedWith(
      "States must have at least 1 seat"
    );
  });
  it("Should throw error that result cannot be tie", async function () {
  ...
  it("Should throw that election has already ended", async function () {
    const stateResults = ["California", 1000, 900, 32];
    expect(usElection.submitStateResult(stateResults)).to.be.revertedWith(
